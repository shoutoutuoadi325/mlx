...
/**
 * Fetch the current bypass list for a repo. Returns a Set of logins.
 */
async function fetchBypassList(owner, repo, token) {
  ...
  if (res.status === 404) {
    // No bypass list configured yet; treat as empty.
    return new Set();
  }
...
/**
 * Add a batch of users to the bypass list in a single PUT call.
...
/**
 * Remove a batch of users from the bypass list in a single DELETE call.
 * No-op if the list is empty.
 */
...
/**
 * Compute the diff between the desired (eligible) set and the current
 * bypass list: who needs to be added, who needs to be removed.
 */
...
async function updateBypassList({ owner, repo, token, dryRun = false }) {
  ...
  const eligibleLogins = new Set(eligibleUsers.map((u) => u.login));
